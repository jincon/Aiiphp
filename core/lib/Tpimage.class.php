...
class Tpimage {
    ...
    static function getFileSize($url){
        ...
        return isset($matches[1])&&is_numeric($matches[1])?$matches[1]:0;
    }
    ...
    static function getImageInfo($img) {
        ...
        if ($imageInfo !== false) {
            ...
            $imageSize = self::getFileSize($img); //fix
         $info = array(
                "width" => $imageInfo[0],
                "height" => $imageInfo[1],
                "type" => $imageType,
     ...
     * @return void
     */
    static public function water($source, $water, $savename=null, $alpha=80, $posX=0, $posY=0) {
        //检查文件是否存在
        ...
        //图像位置,默认为右下角右对齐
        $posX = $posX?$posX:($sInfo["width"] - $wInfo["width"]);
        $posY = $posY?$posY:($sInfo["height"] - $wInfo["height"]);
    ...
    function showImg($imgFile, $text='', $x='10', $y='10', $alpha='50') {
        ...
        $info = Tpimage::getImageInfo($imgFile);
        if ($info !== false) {
            ...
            if ($im) {
                ...
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = Tpimage::getImageInfo($text);
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Tpimage::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
     ...
     * @return string
     */
    static function buildString($string, $size=array(), $filename='', $type='png', $disturb=1, $border=true) {
        if (is_string($size))
            $size = explode(',', $size);
        $width = $size[0];
        $height = $size[1];
        ...
        Tpimage::output($im, $type, $filename);
    }
